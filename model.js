...
exports.add = (question, answer) => {
	quizzes.push({
		// En caso de que sea indefinido, pongo unas comillas y le hago un trim
		// para quitar los espacios que tenga por delante y por detrás.
...
exports.update = (id, question, answer) => {
	...
	// Si el parámetro id no es válido, lanzo error.
	if (typeof quiz === "undefined") {
		throw new Error(`El valor del parámetro id no es válido.`);
	}
	// En la posicion id del array quizzes, quiero quitar 1 elemento y poner
	// lo que hay a continuación.
	quizzes.splice(id, 1, {
		question: (question || "").trim(),
		answer: (answer || "").trim()
	});
...
exports.deleteByIndex = id => {
	...
	// Vete al array y en la posición id borras 1 elemento.
	quizzes.splice(id, 1);
	save();
};
